...
const LIST_COLS = 'id, title, content, tags, source_url, summary, clip_id, created_at, updated_at';
...
export default async function documentRoutes(app: FastifyInstance) {
  ...
  app.get('/api/documents', async () =>
    db.prepare(`SELECT ${LIST_COLS} FROM documents ORDER BY updated_at DESC`).all());
  ...
  app.post('/api/documents', async (req, reply) => {
    ...
    db.prepare(
      `INSERT INTO documents (id,title,content,content_text,tags,source_url,summary,created_at,updated_at)
       VALUES (?,?,?,?,?,?,?,?,?)`,
    ).run(id, b.title.trim(), content, stripToText(content),
      b.tags ?? '', b.source_url ?? '', b.summary ?? '', ts, ts);
    rebuildDocsFts();
